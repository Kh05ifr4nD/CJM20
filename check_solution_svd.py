...
print(
    ...
    -np.log(largest_value) / np.log(2),
)

print(
    "\nFinally estimate it through random samples to make sure we haven't made a mistake"
...
for _ in range(100):
    ...
    inp /= np.sum(inp**2, axis=1, keepdims=True) ** 0.5
    inp *= np.sum((np.ones(A1[0].shape[0]) * 2) ** 2) ** 0.5
...
print(
    "Fewest number of bits of precision over",
    len(ls),
    "random samples:",
    -np.log(np.max(ls)) / np.log(2),
)
