...
def get_second_grad_unsigned(x, direction, eps, eps2):
    ...
    """
    grad_value = get_grad(x + direction * eps, direction, eps2) + get_grad(
        x - direction * eps, -direction, eps2
    )
            ...
            for i in N[j]:
                both_ratio.append(
                    get_second_grad_unsigned(
                        point, (basis(i) + basis(N[j][0])) / 2, eps, eps / 3
                    )
                )
...
def get_ratios_lstsq(LAYER, critical_points, N, known_T, debug=False, eps=1e-5):
    ...
    for i, point in enumerate(critical_points):
        ...
        for i in range(np.sum(known_T.forward(point) != 0) + 2):
            ...
            if len(ys) > 0:
                ...
                both_ratio_val = get_second_grad_unsigned(
                    point, (d + d_matrix[0]) / 2, eps, eps / 3
                )
        ...
        soln, *rest = np.linalg.lstsq(
            np.array(h_matrix, dtype=np.float32), np.array(ys, dtype=np.float32), 1e-5
        )
