...
def is_on_following_layer(known_T, known_A, known_B, point):
    ...
    def is_on_prior_layer(query):
        ...
        if any(
            np.min(np.abs(layer)) < 1e-5 for layer in known_T.get_hidden_layers(query)
        ):
            return True
    ...
    for tol in range(10):
        ...
        _, high = binary_search_towards(
            known_T, known_A, known_B, point, initial_signs, go_direction
        )

        if CHEATING:
            print(
                cheat_get_inner_layers(point + go_direction * high / 2)[np.argmin(ls)]
            )

        point_in_same_polytope = point + (high * 0.999 - 1e-4) * go_direction

        print("high", high)

        solutions = do_better_sweep(
            point_in_same_polytope, normal, -1e-4 * high, 1e-4 * high, known_T=known_T
        )
        if len(solutions) >= 1:
...
def find_plane_angle(
    ...
    while True:
        x_dir_base = np.sign(np.random.normal(size=DIM)) / DIM**0.5
        y_dir_base = np.sign(np.random.normal(size=DIM)) / DIM**0.5
    ...
    for stepsize in start + list(range(init_step, MAX)):
        ...
        top = do_better_sweep(
            multiple_intersection_point + x_dir, y_dir, -1, 1, known_T=known_T
        )
        bot = do_better_sweep(
            multiple_intersection_point - x_dir, y_dir, -1, 1, known_T=known_T
        )
        left = do_better_sweep(
            multiple_intersection_point + y_dir, x_dir, -1, 1, known_T=known_T
        )
        right = do_better_sweep(
            multiple_intersection_point - y_dir, x_dir, -1, 1, known_T=known_T
        )
        ...
        if (
            len(intersections) == 0 and stepsize > 15
        ):  # or (len(intersections) == 3 and stepsize > 5):
        ...
        if (len(intersections) > 4 or stepsize > 20) and len(
            prev_iter_intersections
        ) >= 2:
            ...
            for critical_point, is_prior_layer_zero in zip(
                ...
                vs = known_T.extend_by(known_A, known_B).get_hidden_layers(
                    critical_point
                )
                # print("VS IS", vs)
...
def binary_search_towards_slow(
    known_T, known_A, known_B, start_point, initial_signs, go_direction, maxstep=1e6
):
    low = 0
    high = maxstep
    while high - low > 1e-8:
    ...
    next_signs = get_polytope_at(
        known_T, known_A, known_B, start_point + low * go_direction
    )
...
def binary_search_towards(
    ...
    delta_hidden_np = (
        np.array(plus_T.get_hidden_layers(start_point + 1e-6 * go_direction, flat=True))
        - initial_hidden
    ) * 1e6
    #
    ...
    if (
        PREV_GRAD is None
        or PREV_GRAD[0] is not known_T
        or PREV_GRAD[1] is not known_A
        or PREV_GRAD[2] is not known_B
    ):
        ...
        def grads(start_point, go_direction):
            return jnp.array(
                [
                    jnp.dot(g(start_point, i), go_direction)
                    for i in range(initial_hidden.shape[0])
                ]
            )
...
def follow_hyperplane(
    ...
    def choose_new_direction_from_minimize(previous_axis):
        ...
        if len(history) == 0:
            ...
            print(
                "Progress", "%.1f" % int(np.mean(neuron_consistency != 0) * 100) + "%"
            )
            ...
            if which_to_change != previous_axis:
                if (
                    ...
                    and neuron_consistency[previous_axis]
                    == neuron_consistency[which_to_change]
                ):
                    ...
                    valid_axes = np.where(
                        neuron_consistency == neuron_consistency[which_to_change]
                    )[0]
            ...
            fn = (
                min
                if neuron_positive_count[which_to_change]
                > neuron_negative_count[which_to_change]
                else max
            )
            arg_fn = (
                np.argmin
            ...
            print(
                "Changing",
                which_to_change,
                "to flip sides because mean is",
                mean_plus_neuron_value[which_to_change],
            )

        val = matmul(
            known_T.forward(new_start_point, with_relu=True), known_A, known_B
        )[which_to_change]
        ...
        for _ in range(1000):
            random_dir = np.random.normal(size=DIM)
            perp_component = (
                np.dot(random_dir, new_perp_dir)
                / (np.dot(new_perp_dir, new_perp_dir))
                * new_perp_dir
            )
            parallel_dir = random_dir - perp_component
    ...
    while True:
        ...
        which_polytope = get_polytope_at(
            known_T, known_A, known_B, start_point, False
        )  # [-1 1 -1]
        hidden_vector = get_hidden_at(
            known_T, known_A, known_B, LAYER, start_point, False
        )
        ...
        if (
            np.all(neuron_positive_count > 0) and np.all(neuron_negative_count > 0)
        ) or (only_need_positive and np.all(neuron_positive_count > 0)):
        ...
        try:
            ...
            perp_dir = get_ratios_lstsq(
                0, [start_point], [range(DIM)], KnownT([], []), eps=1e-5
            )[0].flatten()
        ...
        start_point, multiple_intersection_point, new_change_axis = (
            choose_new_direction_from_minimize(current_change_axis)
        )

        if new_change_axis != current_change_axis:
            start_point, multiple_intersection_point, current_change_axis = (
                choose_new_direction_from_minimize(None)
            )
        ...
        if step_distance > 1 or True:
            mid_point = (
                ...
                * towards_multiple_direction
                / np.sum(towards_multiple_direction**2) ** 0.5
                + start_point
            )

            random_dir = np.random.normal(size=DIM)

            mid_points = do_better_sweep(
                mid_point,
                perp_dir / np.sum(perp_dir**2) ** 0.5,
                low=-1e-3,
                high=1e-3,
                known_T=known_T,
            )
            ...
            if len(mid_points) > 0:
                mid_point = mid_points[
                    np.argmin(np.sum((mid_point - mid_points) ** 2, axis=1))
                ]
                ...
                multiple_intersection_point = (
                    towards_multiple_direction * high + start_point
                )
        ...
        val = matmul(known_T.forward(success, with_relu=True), known_A, known_B)[
            new_change_axis
        ]
        ...
        if stepsize < 10:
            ...
            while high - low > 1e-2:
                ...
                print(
                    "polytope at",
                    mid,
                    sign_to_int(next_signs),
                    "%x" % (sign_to_int(next_signs) ^ sign_to_int(initial_signs)),
                )
                if initial_signs == next_signs:
                    low = mid
                else:
...
def solve_contractive_sign(known_T, weight, bias, LAYER):
    ...
    def get_preimage(hidden):
        ...
        for i, (my_A, my_B) in reversed(
            list(enumerate(zip(known_T.A + [weight], known_T.B + [bias])))
        ):
            if i == 0:
                res = scipy.optimize.lsq_linear(
                    my_A.T, preimage - my_B, bounds=(-np.inf, np.inf)
                )
            else:
                res = scipy.optimize.lsq_linear(
                    my_A.T, preimage - my_B, bounds=(0, np.inf)
                )
    ...
    for axis in range(len(hidden)):
        ...
        print("One of these should be small", inverted_if_small, not_inverted_if_small)
...
def solve_layer_sign(
    ...
    if already_checked_critical_points:
        for point in get_critical_point:
            ...
            pts.append(point)
            which_polytope = get_polytope_at(
                known_T, known_A0, known_B0, point, False
            )  # [-1 1 -1]
            hidden_vector = get_hidden_at(
                known_T, known_A0, known_B0, LAYER, point, False
            )
            if CHEATING:
                layers = cheat_get_inner_layers(point)
                print(
                    "have", [(np.argmin(np.abs(x)), np.min(np.abs(x))) for x in layers]
                )
    ...
    while True:
        ...
        while len(pts) > prev_count + 2:
            ...
            if done and only_need_positive:
                ...
                break
            if (
                np.all(neuron_positive_count > 0)
                and np.all(neuron_negative_count > 0)
                or (only_need_positive and np.all(neuron_positive_count > 0))
            ):
        ...
        if len(pts) < MAX_POINTS / 2 and good == False:
            ...
            if already_checked_critical_points:
                ...
                which_point = initial_points[
                    np.random.randint(0, len(initial_points) - 1)
                ]
            else:
                which_point = next(get_critical_point)
        else:
    ...
    all_res = pool[0].map(
        is_solution_map, [(bound, extra_args_tup) for bound in bounds]
    )
