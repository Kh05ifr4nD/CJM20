...
def get_hidden_at(known_T, known_A, known_B, LAYER, x, prior=True):
    ...
    if prior:
        ...
        which_activation = []
    which_activation += list(
        matmul(known_T.forward(x[np.newaxis, :], with_relu=True), known_A, known_B)[0]
    )
...
def check_quality(layer_num, extracted_normal, extracted_bias, do_fix=False):
    ...
    for i in range(neuron_count[layer_num + 1]):
        ...
        print(
            ...
            np.min(gaps) ** 0.5,
            "ratio",
            ratios[np.argmin(gaps)],
        )
        print(
            "Bias check",
            (
                __cheat_B[layer_num][np.argmin(gaps)]
                - extracted_bias[i] * ratios[np.argmin(gaps)]
            ),
        )
        ...
        if min(gaps) > 1e-2:
            ...
            print(
                "\t  Got:",
                " ".join(
                    "%.04f" % x for x in extracted_normal[:, i] / extracted_normal[0, i]
                ),
            )
            print(
                "\t Real:",
                " ".join(
                    "%.04f" % x
                    for x in __cheat_A[layer_num][:, np.argmin(gaps)]
                    / __cheat_A[layer_num][0, np.argmin(gaps)]
                ),
            )
