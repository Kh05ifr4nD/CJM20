...
def process_block(ratios, other_ratios):
    ...
    differences = differences / jnp.abs(
        ratios[:, jnp.newaxis, :]
    ) + differences / jnp.abs(other_ratios[jnp.newaxis, :, :])

    close = differences < BLOCK_ERROR_TOL * jnp.log(ratios.shape[1])

    pairings = jnp.sum(close, axis=2) >= max(
        MIN_SAME_SIZE, BLOCK_MULTIPLY_FACTOR * (np.log(ratios.shape[1]) - 2)
    )
...
def graph_solve(all_ratios, all_criticals, expected_neurons, LAYER, debug=False):
    ...
    ratios_group = [
        all_ratios_f32[i : i + 1000] for i in range(0, len(all_ratios), 1000)
    ]
    criticals_group = [
        all_criticals[i : i + 1000] for i in range(0, len(all_criticals), 1000)
    ]
    ...
    for batch_index, (criticals, ratios) in enumerate(
        zip(criticals_group, ratios_group)
    ):
        print(batch_index)
    ...
    graph.add_edges_from(
        [(i, j) for i, js in enumerate(all_pairings) for j in js if abs(i - j) > 1]
    )
    ...
    if CHEATING:
        print(
            "Total (unmatched) examples found:",
            sorted(
                collections.Counter(
                    which_is_zero(LAYER, cheat_get_inner_layers(all_criticals))
                ).items()
            ),
        )
    ...
    print(
        "Graph search found",
        len(components),
        "different components with the following counts",
        list(map(len, sorted_components)),
    )
    ...
    if CHEATING:
        which_neurons = [
            collections.Counter(
                which_is_zero(
                    LAYER, cheat_get_inner_layers(all_criticals[list(orig_component)])
                )
            )
    ...
    while previous_num_components > len(sorted_components):
        ...
        components = [
            sum([list(candidate_components[y]) for y in comp], [])
        ...
        print(
            "After re-doing the graph, the component counts is",
            len(components),
            "with items",
            list(map(len, sorted_components)),
        )
        ...
        if CHEATING:
            which_neurons = [
                collections.Counter(
                    which_is_zero(
                        LAYER,
                        cheat_get_inner_layers(all_criticals[list(orig_component)]),
                    )
                )
                for orig_component in sorted_components
            ]
    ...
    for c_count, component in enumerate(sorted_components):
        ...
        guessed_row, normalize_axis, normalize_error = ratio_normalize(
            possible_matrix_rows
        )

        print(
            "The guessed error in the computation is",
            normalize_error,
            "with",
            len(component),
            "witnesses",
        )
        if normalize_error > 0.01 and len(component) <= 5:
            print(
        ...
        if len(resulting_rows):
            ...
            scaled_resulting_rows /= scaled_resulting_rows[
                :, normalize_axis : normalize_axis + 1
            ]
        ...
        if CHEATING:
            ...
            if (error > 1e-4 * len(guessed_row) and debug) or do_print_err:
                ...
                print(
                    ...
                    " ".join(
                        "%2.3f" % (np.abs(x - y))
                        for x, y in zip(guessed_row, actual_row)
                    ),
                )
                # print("scale", scale)
                print("--")
                for row in possible_matrix_rows:
                    print(
                        "posbl",
                        " ".join("%2.3f" % x for x in row / row[normalize_axis]),
                    )
                print("--")

                scale = 10 ** int(
                    np.round(
                        np.log(np.nanmedian(np.abs(possible_matrix_rows))) / np.log(10)
                    )
                )
        ...
        if np.any(np.isnan(guessed_row)) and c_count < expected_neurons:
            print(
                "Got NaN, need more data",
                len(component) / sum(map(len, components)),
                1 / sizes[LAYER + 1],
            )
            if len(component) >= 3:
                if c_count < expected_neurons:
    ...
    if (
        len(resulting_rows) + skips_because_of_nan < expected_neurons
        and len(all_ratios) < DEAD_NEURON_THRESHOLD
    ):
        print(
            "We have not explored all neurons. Do more random search",
            len(resulting_rows),
            skips_because_of_nan,
            expected_neurons,
        )
        raise AcceptableFailure(
            partial_solution=(
                np.array(resulting_rows),
                np.asanyarray(resulting_examples, dtype="object"),
            )
        )
    else:
...
def ratio_normalize(possible_matrix_rows):
    ...
    while (
        np.sum(np.isnan(ratio_evidence)) < last_nan_count
        or last_total_cost < np.sum(errors) * 0.9
    ) and False:
        ...
        for i in range(len(best_indexs)):
            cost_i_over_k_new.append(
                cost_i_over_k[i].T[np.arange(len(best_indexs)), best_indexs[i]]
            )
...
def gather_ratios(critical_points, known_T, check_fn, LAYER, COUNT):
    ...
    for point in critical_points:
        ...
        if CHEATING:
            print(
                "What layer is this neuron on (by cheating)?",
                [
                    (np.min(np.abs(x)), np.argmin(np.abs(x)))
                    for x in cheat_get_inner_layers(point)
                ],
            )
        ...
        for EPS in [GRAD_EPS, GRAD_EPS / 10, GRAD_EPS / 100]:
            try:
                normal = get_ratios_lstsq(
                    LAYER, [point], [range(DIM)], known_T, eps=EPS
                )[0].flatten()
...
def compute_layer_values(critical_points, known_T, LAYER):
    ...
    def check_fn(point):
        if partial_weights is None:
            return True
        hidden = matmul(
            known_T.forward(point, with_relu=True), partial_weights.T, partial_biases
        )
    ...
    while True:
        print(
            "At this iteration I have",
            len(this_layer_critical_points),
            "critical points",
        )
        ...
        this_layer_critical_points.extend(
            gather_ratios(reuse_critical_points(), known_T, check_fn, LAYER, COUNT)
        )
        ...
        for i, (ratio1, point1) in enumerate(this_layer_critical_points):
            for ratio2, point2 in this_layer_critical_points[i + 1 :]:
                if np.sum((point1 - point2) ** 2) ** 0.5 < 1e-10:
                    break
            else:
        ...
        print(
            "After filtering duplicates we're down to ",
            len(this_layer_critical_points),
            "critical points",
        )
        ...
        try:
            critical_groups, extracted_normals = graph_solve(
                [x[0] for x in this_layer_critical_points],
            ...
            print(
                "Graph solving failed because we didn't explore all sides of at least one neuron"
            )
            print(
                "Fall back to the hyperplane following algorithm in order to get more data"
            )
            ...
            _, more_critical_points = sign_recovery.solve_layer_sign(
                prev_T,
                known_T.A[-1],
                known_T.B[-1],
                mine(e.data),
                LAYER - 1,
                already_checked_critical_points=True,
                only_need_positive=True,
            )

            print("Add more", len(more_critical_points))
            this_layer_critical_points.extend(
                gather_ratios(more_critical_points, known_T, check_fn, LAYER, 1e6)
            )
            print("Done adding")
            ...
            if "partial_solution" in dir(e):
                if len(e.partial_solution[0]) > 0:
                    ...
                    if CHEATING:
                        print(
                            ...
                            np.argmin(
                                np.abs(
                                    cheat_get_inner_layers(
                                        [x[0] for x in corresponding_examples]
                                    )[LAYER]
                                ),
                                axis=1,
                            ),
                    ...
                    partial_biases = []
                    for weight, examples in zip(
                        partial_weights, corresponding_examples
                    ):
    ...
    extracted_bias = [
        matmul(
            known_T.forward(point_per_class[i], with_relu=True),
            extracted_normals[:, i],
            c=None,
        )
        for i in range(neuron_count[LAYER + 1])
    ]
    ...
    if CHEATING:
        ...
        for i in range(len(extracted_bias)):
            which_idx = np.argmin(
                np.sum(np.abs(real_scaled - extracted_scaled[:, [i]]), axis=0)
            )
        ...
        print(
            ...
            np.sum(np.abs(extracted_normals * mask - A[LAYER][:, reorder_rows])),
        )
    else:
