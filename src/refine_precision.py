    ...
    def get_more_points(NUM):
        ...
        while len(critical_points) <= NUM:
            ...
            for step in range(5000):
                ...
                if step % 1000 == 0:
                    lr *= 0.5
                    init, opt_update, get_params = (
                        jax.example_libraries.optimizers.adam(lr)
                    )
            ...
            for point in points:
                ...
                if LAYER > 0:
                    ...
                    if (
                        min(np.min(np.abs(x)) for x in known_T.get_hidden_layers(point))
                        < 1e-4
                    ):
                        print("is on prior")
                        continue
                ...
                tmp = query_count
                solution = do_better_sweep(
                    offset=point, low=-stepsize, high=stepsize, known_T=known_T
                )
                ...
                if len(solution) == 0:
                    ...
                    if (
                        min(np.min(np.abs(x)) for x in this_hidden_vec)
                        > np.abs(this_hidden) * 0.9
                    ):
    ...
    for _ in range(1):
        ...
        for iteration in range(upto):
            ...
            if iteration % 2 == 0 or True:
                ...
                randn = np.random.choice(
                    len(hidden_layer),
                    min(len(hidden_layer), hidden_layer.shape[1] + 20),
                    replace=False,
                )
                soln, _ = trim(
                    hidden_layer[randn],
            ...
            if quality < 1e-10:
                break
            if quality < 1e-10 and iteration > 1e4:
                break
            if quality < 1e-8 and iteration > 1e5:
                break
        ...
        print("Compare", np.median(np.abs(crit_val_1)), best[1])
...
def improve_layer_precision(LAYER, known_T, known_A, known_B):
    ...
    out = map(
        improve_row_precision,
        [
            (LAYER, known_T, known_A, known_B, row, False)
            for row in range(neuron_count[LAYER + 1])
        ],
    )
    new_A, new_B = zip(*out)
